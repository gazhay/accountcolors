...
var accountColors3Pane = {
  ...
  prefsObserver: {
    ...
    unregister: function () {
      ...
      accountColors3Pane.prefs.removeObserver("", this);
    },
  ...
  mailPrefsObserver: {
    ...
    unregister: function () {
      ...
      accountColors3Pane.mailPrefs.removeObserver("", this);
    },
  ...
  onUnload: function () {
    accountColors3Pane.prefsObserver.unregister();
    accountColors3Pane.mailPrefsObserver.unregister();

    window.removeEventListener("load", accountColors3Pane.folderPane, false);

    document.getElementById("folderTree").removeEventListener("select", accountColors3Pane.threadPane, false);
    document.getElementById("folderTree").removeEventListener("mousemove", accountColors3Pane.onMouseMove, false);
    document.getElementById("folderTree").removeEventListener("dragover", accountColors3Pane.onDragOver, false);
    document.getElementById("folderTree").removeEventListener("dragleave", accountColors3Pane.onDrop, false);
    document.getElementById("folderTree").removeEventListener("drop", accountColors3Pane.onDrop, false);

    document.getElementById("tabmail").removeEventListener("load", accountColors3Pane.messagePane, true);
    document.getElementById("threadTree").removeEventListener("mousemove", accountColors3Pane.onMouseMove, false);

    // Clear on unload.
    accountColors3Pane.messagePane(null, true);

    if (gFolderTreeView.accountcolors_old_getCellProperties) {
      gFolderTreeView.getCellProperties = gFolderTreeView.accountcolors_old_getCellProperties;
      delete gFolderTreeView.accountcolors_old_getCellProperties;
    }

    if (gFolderTreeView.accountcolors_old_getRowProperties) {
      gFolderTreeView.getRowProperties = gFolderTreeView.accountcolors_old_getRowProperties;
      delete gFolderTreeView.accountcolors_old_getRowProperties;
    }
  },
    ...
    function checkReady() {
      ...
      try {
        for (i = 0; i < document.styleSheets.length; i++) {
          if (document.styleSheets[i].href == "chrome://accountcolors-skin/content/accountcolors-3panewindow-generated.css") {
      ...
      if (ready)
        window.setTimeout(function () {
          accountColors3Pane.initializePanes();
        }, 0); /* break execution */
    }
  },
  ...
  validatePrefs: function () {
    ...
    for (acc = 0; acc < accountColorsUtilities.getLength(accounts); acc++) {
      ...
      if (accountColorsUtilities.getLength(identities) == 0) {
        ...
        if (!accountColors3Pane.prefs.prefHasUserValue(account.key + "-fontcolor") || !accountColors3Pane.prefs.prefHasUserValue(account.key + "-bkgdcolor")) {
        ...
        for (id = 0; id < accountColorsUtilities.getLength(identities); id++) {
          ...
          if (!accountColors3Pane.prefs.prefHasUserValue(identity.key + "-fontcolor") || !accountColors3Pane.prefs.prefHasUserValue(identity.key + "-bkgdcolor")) {
    ...
    for (acc = 0; acc < accountsprefs.length; acc++) {
      ...
      if (!accountColors3Pane.mailPrefs.prefHasUserValue("account." + accountkey + ".server") || accountColors3Pane.mailPrefs.prefHasUserValue("account." + accountkey + ".identities")) {
  ...
  folderSetup: function () {
    ...
    gFolderTreeView.getCellProperties = function new_getCellProperties(row, col) {
      ...
      if (col.id == "folderNameCol") {
        ...
        if ((accountColors3Pane.prefs.getBoolPref("folder-colorbkgd") && gFolderTreeView._rowMap[row]._folder.isServer) || (accountColors3Pane.prefs.getBoolPref("folder-colorfldbkgd") && !gFolderTreeView._rowMap[row]._folder.isServer)) {
    ...
    gFolderTreeView.getRowProperties = function new_getRowProperties(row) {
      var props, server, account, accountidkey;
      var bkgdcolor, red, green, blue, brightness;
  ...
  threadColumnHandler: {
    ...
    getCellProperties: function (row, col) {
      ...
      if (accountColors3Pane.prefs.getBoolPref("thread-hdraccount")) {
  ...
  messagePane: function (event, clear) {
    ...
    if (accountColors3Pane.prefs.getBoolPref("message-hdraccount")) {
      /* color using account in message header */
      accountkey = accountColorsUtilities.getAccountKey(msgHdr); /* null string if sent message */
      account = accountColors3Pane.accountManager.getAccount(accountkey);
