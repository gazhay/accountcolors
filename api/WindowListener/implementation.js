...
var WindowListener = class extends ExtensionCommon.ExtensionAPI {
  ...
  add68PrefsEntry(event) {
    ...
    let iconSizes = this.extension.manifest.icons ? Object.keys(this.extension.manifest.icons) : [];
    ...
    let entry = icon ? event.target.ownerGlobal.MozXULElement.parseXULToFragment(`<menuitem class="menuitem-iconic" id="${id}" image="${icon}" label="${name}" />`) : event.target.ownerGlobal.MozXULElement.parseXULToFragment(`<menuitem id="${id}" label="${name}" />`);
      ...
      default: {
        ...
        for (let card of cards) {
          ...
          if (card.addon.id == this.extension.id) {
            let optionsMenu = (this.getThunderbirdVersion().major > 78 && this.getThunderbirdVersion().major < 88) || (this.getThunderbirdVersion().major == 78 && this.getThunderbirdVersion().minor < 10) || (this.getThunderbirdVersion().major == 78 && this.getThunderbirdVersion().minor == 10 && this.getThunderbirdVersion().revision < 2);
            if (optionsMenu) {
              ...
              if (card.addon.isActive && !addonOptionsLegacyEntry) {
                ...
                addonOptionsEntry.parentNode.insertBefore(addonOptionsLegacyEntry, addonOptionsEntry);
  ...
  getMessenger(context) {
    ...
    function getStorage() {
      ...
      try {
        ...
        localstorage.local.get = (...args) => localstorage.local.callMethodInParentProcess("get", args);
        localstorage.local.set = (...args) => localstorage.local.callMethodInParentProcess("set", args);
        localstorage.local.remove = (...args) => localstorage.local.callMethodInParentProcess("remove", args);
        localstorage.local.clear = (...args) => localstorage.local.callMethodInParentProcess("clear", args);
      } catch (e) {
    ...
    for (let api of apis) {
      switch (api) {
        ...
        default:
          XPCOMUtils.defineLazyGetter(messenger, api, () => context.apiCan.findAPIPath(api));
      }
    }
    return messenger;
  }
  ...
  getAPI(context) {
    ...
    if (context.viewType != "background") throw new Error("The WindowListener API may only be called from the background page.");
          ...
          if (!aTab.pageLoaded) {
            ...
            await new Promise((resolve) => {
              let reporterListener = {
                ...
                onStateChange() {},
                onProgressChange() {},
                onLocationChange(/* in nsIWebProgress*/ aWebProgress, /* in nsIRequest*/ aRequest, /* in nsIURI*/ aLocation) {
    ...
    return {
      WindowListener: {
        ...
        registerOptionsPage(optionsUrl) {
          self.pathToOptionsPage = optionsUrl.startsWith("chrome://") ? optionsUrl : context.extension.rootURI.resolve(optionsUrl);
        },
        ...
        registerWindow(windowHref, jsFile) {
          ...
          if (!self.registeredWindows.hasOwnProperty(windowHref)) {
            ...
            let path = jsFile.startsWith("chrome://") ? jsFile : context.extension.rootURI.resolve(jsFile);
        ...
        registerStartupScript(aPath) {
          self.pathToStartupScript = aPath.startsWith("chrome://") ? aPath : context.extension.rootURI.resolve(aPath);
        },

        registerShutdownScript(aPath) {
          self.pathToShutdownScript = aPath.startsWith("chrome://") ? aPath : context.extension.rootURI.resolve(aPath);
        },
          ...
          if (urls.length > 0) {
            ...
            ExtensionSupport.registerWindowListener("injectListener_" + self.uniqueRandomID, {
              ...
              async onLoadWindow(window) {
                ...
                if (window.location.href == "chrome://messenger/content/messenger.xul" || window.location.href == "chrome://messenger/content/messenger.xhtml") {
                  if (self.pathToOptionsPage) {
                    if (self.getThunderbirdVersion().major < 78) {
                      ...
                      self.setupAddonManager(self.getAddonManagerFromWindow(window), true);
                ...
                if (browserElements.length > 0) {
                  ...
                  for (let element of browserElements) {
                    if (self.registeredWindows.hasOwnProperty(element.getAttribute("src"))) {
                      ...
                      window[self.uniqueRandomID]._mObserver.observe(element, {
                        attributes: true,
                        childList: false,
                        characterData: false,
                      });
              ...
              for (const persistentNode of elements[i].querySelectorAll("[persist]")) {
                for (const persistentAttribute of persistentNode.getAttribute("persist").trim().split(" ")) {
                  if (Services.xulStore.hasValue(uri, persistentNode.id, persistentAttribute)) {
                    persistentNode.setAttribute(persistentAttribute, Services.xulStore.getValue(uri, persistentNode.id, persistentAttribute));
                  }
                }
              }
  ...
  onShutdown(isAppShutdown) {
    ...
    if (urls.length > 0) {
      for (let window of Services.wm.getEnumerator(null)) {
        ...
        if (this.pathToOptionsPage && (window.location.href == "chrome://messenger/content/messenger.xul" || window.location.href == "chrome://messenger/content/messenger.xhtml")) {
